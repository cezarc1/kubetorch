...
def kt_teardown(
    ...
    prefix: str = typer.Option(None, "-p", "--prefix", help="Tear down all services with given prefix"),
    namespace: str = typer.Option(
        globals.config.namespace,
        "-n",
        "--namespace",
    ...
    """
    from kubetorch import config
    from kubetorch.resources.compute.utils import delete_resources_for_services, fetch_resources_for_teardown
    ...
    if not yes and not force:  # if --force is provided, we don't need additional confirmation
        resources = fetch_resources_for_teardown(
            ...
            target=name,
            prefix=prefix,
            username="sashab" if teardown_all else None,
            exact_match=exact_match,
        )

        services = dict(resources["services"])
    ...
    try:
        username = config.username if not prefix else None
        ...
        deleted_data_store_keys: dict = delete_result.get("data_store_keys")
        deleted_services = {
        ...
        deleted_pools = deleted_services.pop("pools", None)
        assert deleted_pools
        # TODO: should we include pools in the kt teardown output?
        ...
        for service_type in deleted_services:
            ...
            service_deletion_msg = f"{force_deleted_msg_prefix} [reset]{service_type}"

            for resource in deleted_resources:
                console.print(f"{service_deletion_msg} [blue]{resource}[/blue]")
                resource_data_store_keys = deleted_data_store_keys.get(resource)
                if resource_data_store_keys:
                    console.print(f'✓ Deleted cached data for {resource}"')
