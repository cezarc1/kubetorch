# Dynamic World Size with Persistent Checkpoints
```{admonition} Adapted
:class: kt-status kt-status-adapted

**Adapted** for Kubetorch `0.5.2`. Updated for the current fork and checked statically; cluster execution is not yet recorded.

Requires: `Multi-node GPU`. [View source](https://github.com/cezarc1/kubetorch/blob/main/examples/tutorials/fault_tolerance/dynamic_world_size_rs.py).
```
