...
def _load_smoke_module():
    script_path = Path(__file__).parents[1] / "scripts" / "edgellm_quant_export_smoke.py"
    spec = importlib.util.spec_from_file_location("edgellm_quant_export_smoke", script_path)
...
def test_artifact_prefix_uses_model_id_and_quantization():
    ...
    assert smoke.artifact_prefix("Qwen/Qwen3-ASR-0.6B", "int8_sq") == "Qwen3-ASR-0.6B-int8_sq"
    assert smoke.artifact_prefix("/models/Qwen3-ASR-1.7B", "int4_awq") == "Qwen3-ASR-1.7B-int4_awq"
...
def test_artifact_upload_filters_bypass_repo_gitignore(monkeypatch):
    ...
    with smoke.artifact_upload_filters():
        ...
        assert smoke.os.environ["KT_RSYNC_FILTERS"] == smoke.artifact_rsync_exclude_override()
