...
from qwen3_asr_orin.edgellm_results import (
...
def main(argv: list[str] | None = None) -> None:
    ...
    prepare = subparsers.add_parser(
        "prepare-manifest", help="Download a small LibriSpeech + FLEURS manifest"
    )
    ...
    bench = subparsers.add_parser(
        "bench", help="Benchmark an OpenAI-compatible audio transcription endpoint"
    )
    ...
    quant = subparsers.add_parser(
        "quantize-modelopt", help="Write or run the ModelOpt INT8 quantization spec"
    )
    ...
    trt_bundle.add_argument(
        "--format", choices=("fp16", "int8", "int4"), default="int8"
    )
    ...
    rescore = subparsers.add_parser(
        "rescore-edgellm-results", help="Recompute WER for saved Edge-LLM samples"
    )
    ...
    compare.add_argument(
        "--run", action="append", required=True, help="Run summary in label=path format"
    )
    ...
    if args.command == "prepare-manifest":
        languages = tuple(
            language.strip()
            for language in args.fleurs_languages.split(",")
            if language.strip()
        )
