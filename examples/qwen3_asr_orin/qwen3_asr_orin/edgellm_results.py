...
DEFAULT_LANGUAGE_PREFIX_REGEX = r"^language\s+(?:English|German|Deutsch|Spanish|Español|French|Français)\s*"


def strip_hypothesis_prefix(text: str, pattern: str = DEFAULT_LANGUAGE_PREFIX_REGEX) -> str:
...
def rescore_samples_file(
    ...
    for row in rows:
        rescored = dict(row)
        rescored["scored_hypothesis"] = strip_hypothesis_prefix(str(row.get("hypothesis", "")), strip_prefix_regex)
        rescored["wer"] = word_error_rate(str(row.get("reference", "")), rescored["scored_hypothesis"])
    ...
    summary = _summary(rescored_rows, samples_path=samples_path, rescored_samples_path=rescored_samples_path)
...
def _summary(rows: list[dict], samples_path: Path, rescored_samples_path: Path) -> dict:
    ...
    return {
        ...
        "mean_latency_seconds": total_latency_seconds / sample_count if sample_count else 0.0,
