...
- [x] **Step 1: Add verified transcript deltas**
...
- DeepSeek OCR: `AsyncLLMEngine` continuously batches requests within each replica; fixed min/max replicas suit a finite batch job; `concurrency` is a per-pod cap while the driver semaphore bounds total outstanding local tasks; `kt run` can move the driver into the cluster.
...
- [x] **Step 2: Remove video declarations and rendering**
